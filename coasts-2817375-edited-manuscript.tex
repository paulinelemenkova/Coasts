...
The Landsat OLI-TIRS satellite missions \hl{measured the} surface reflectivity and \hl{scattering} in the \hl{multi-spectral} bands\hl{,} {which \hl{resulted} in its successful application during \hl{recent} decades} \cite{WULDER2022113195}. Its \hl{capacity} to discriminate between different vegetation types \hl{was} demonstrated in previous investigations~\cite{Ferreira,Montfort,Fatoyinbo,Ribeiro}. In~cloud-prone and rainy areas typical \hl{of} coastal regions of tropical African countries, however, the~use of RS data is restricted by the \hl{atmospheric} effects\hl{,} such as attenuation of signals and \hl{the} cloud coverage during wet \hl{periods}. {Therefore, the~analysis of vegetation should be based on cloud-free images or with \hl{minimized} cloudiness (below 10}\%). {Moreover, the~remaining atmospheric effects should be \hl{minimized} using advanced methods of computer vision. These include, for~instance, \hl{atmospheric} scattering or haze\hl{,} which occur as a result of\hl{ }suspended dust \hl{that accumulates} in relatively dry air. Such effects may lead to \hl{color} distortion \hl{of} the images and worsen image quality through impaired visibility\hl{,} which may affect image analysis. }

As a result, the~processing of such data{ }requires advanced methods of satellite image processing{, e.g.,~removing noise, highlighting edges, \hl{and} improving image contrast through correction of atmospheric effects. This is possible using} \hl{computer vision} and \hl{machine learning} {(ML) } \hl{algorithms} for data processing and visualization\hl{,} {due to \hl{their} impressive computational and spatial analysis capabilities }\cite{LI2023104653,PHAM2023104501,HAN202387}. {The most \hl{important} advantage of ML techniques is that \hl{they are} capable of \hl{automagically} deriving information from\hl{ }existing datasets. Moreover, ML methods apply programming algorithms \hl{that enable them} to excel in complex analysis and \hl{modeling} of geospatial data} \cite{KHAN2022101678}. {In\hl{ }environmental studies, valuable insights \hl{from} ML \hl{include} information extraction from complex terrain and \hl{heterogeneous} landscapes\hl{,} such as coastal regions. Such features of ML facilitate environmental analysis and \hl{enhanced} planning and management of the coastal zones.}

Establishing new methods of Earth Observation data processing, such as \hl{with} ML\hl{,} requires the application of programming approaches \hl{that} enable \hl{the automation of} tasks~\cite{MULLISSA2023113799,Lemenkova202206}. Indeed, the~integrated use of\hl{ }programming scripts, ML capabilities, and cartographic methods \hl{has} provided powerful tools for RS data processing, with the aim of \hl{the} mapping, analysis\hl{,} and monitoring of coastal areas using \hl{the} distinctive sensor responses of land cover types. {Thus, \hl{scripting} and programming \hl{approaches enable overcoming} the ambiguities in image classification resulting from similarities \hl{in the} spectral reflectance \hl{of} various land cover types, and~especially with\hl{ }medium-resolution Landsat data\hl{, maintaining} the \hl{high-automation} streams of \hl{modeling}.} Advancements in computer processing, ML methods and the development of scripting languages, such as Python, used either \hl{alone} or {as }integrated {tools and add-ons }in a Geographic Information System (GIS), have allowed an advanced {approach to} cartography and image processing. Such methods support accurate satellite data processing and analysis, which have {made} dynamic vegetation modeling possible~\cite{Lemenkova202227}. 

%----------- subsection ----------->
\subsection{Related~Works}
Generally speaking, the~accuracy of classification of \hl{the} vegetation types in heterogeneous landscapes using satellite images \hl{mainly depends} on \hl{the} sensitivity of sensor backscattering to\hl{ }{differences} between the phenological characteristics of plant leaves{. Alternatively, they also should be sensitive to the variations in} structure of \hl{the} other land cover types{ }such as bare land or urban areas with dominating impervious surfaces{. Hence, }different \hl{interactions} between the sensor backscatter and the structure of the canopy of landscape patch {can provide \hl{novel} information on ecological structure}. Nevertheless, many recent studies~\cite{rs15235559,WU2023103407,CHEN2023101499,rs15225387,LIN2021102370}\hl{ }noted the evident advantages of using the {ML} methods in image processing. For~instance, ref.~\cite{LIAO2023110231} found that the use of Deep Leaning (DL) modelling in remote sensing data processing could improve \hl{the accuracy of the }classification. 

Earlier studies \hl{have} also reported that \hl{the} classification precision \hl{was significantly} improved \hl{when} the\hl{ }computer vision algorithms of DL for image analysis and interpretation \hl{were applied}~\cite{GUO20231,rs15215121,DOU2021102477}. Similar studies also validated the potential of the use of ML methods in cartographic{ }applications {of RS data processing }for mapping various vegetation classes{. Thus, these methods were used for evaluation of }the optimal combination of geospatial data \hl{used }for separating different land cover classes~\cite{BOULILA2021106014,LIN2024102618}. These and other{ }results {reported in previous studies }also indicated that\hl{ }the use of Python \hl{improves the performance of} the classification and \hl{attains} the required automation and accuracy level in \hl{the process of }image classification~\cite{ROBERTS2022105192,GUO2023e21542,Redoloza}. Nevertheless, \hl{the }improvement and flexibility \hl{are} achieved in the Geographic Resources Analysis Support System (GRASS) GIS software\hl{, which} presents a smart combination of \hl{the existing }Python libraries with cartographic toolset and \hl{a} powerful image processing~functionality. 

{Since the \hl{release of the }first version of\hl{ }GRASS GIS\hl{ }in 1984} \cite{GRASS_GIS_software}, {\hl{this} instrument was constantly improved, which resulted in a \hl{recent }updated version of software 8.3.1\hl{. Nowadays, GRASS GIS presents} a powerful geospatial data processing engine \hl{with an} integrated suite of \hl{the }existing modules for raster and vector data processing. \hl{The }advances in technical progress and rapid development of \hl{the }cartographic instruments along with the \hl{improvements} of programming algorithms resulted in various types of modules of GRASS GIS that include diverse geospatial data processing tools. The~applications of such tools \hl{can be found} in a variety of case studies. To~mention a few of them, the~use of GRASS GIS in geographic studies includes environmental monitoring}~\cite{STRIGARO201668,Lemenkova202322}, {time series and massive data analysis} \cite{JASIEWICZ20111525,Lemenkova202313}, {hydrological modelling} \cite{JASIEWICZ20111162}, {computing landscape diversity, image segmentation} \cite{Lemenkova202320,ROCCHINI201382}, {and geomorphometric modelling} \cite{HOFIERKA2009387}. {These\hl{ }examples\hl{ }prove \hl{the} effectiveness \hl{of the GRASS GIS }for image processing and remote sensing data analysis.}
...
{The goal of this study is to develop a workflow using several modules of GRASS GIS for ML-based Random Forest techniques of image classification. To~this end, the~multi-spectral Landsat \hl{8-9} %MDPI: Please check the full text and confirm if this should be en dah ("--" U+2013).
 OLI/TIRS images are used for land cover mapping of the coastal region of eastern Mozambique that encompasses the Bight of Sofala and cover a time gap from 2015 to 2023. Since the area is located in cloud-prone and rainy region affected by occasional floods, the~data are selected during `dry' period from August to September using optimal atmospheric conditions. Additionally, the~images are corrected to improve \hl{their} quality. In~total, three scenes are used for classification and ML-based processing.}

{The main aim of this study is to evaluate short-term temporal dynamics in the region of Bight of Sofala using advanced methods of cartographic scripting by ML modules of \hl{the} GRASS GIS. The~methodology included \hl{the extraction of the }training data from\hl{ }raster images, supervised ML methods which use ensemble classification\hl{, }regression tree method by RF and cross-validation tasks. Technically, this paper demonstrates the advantages of using the ML algorithms for RS data classification\hl{, as well as advanced approaches of} cartographic mapping for \hl{the }environmental monitoring of coastal areas}. {High robustness, versatility and applicability of the RF algorithm motivated its application for processing satellite images in the shoreline regions \hl{which are notable for highly vulnerable} environmental parameters\hl{. }Due to the sensitivity of RF to noise and outliers in the pixel matrix, its application to image processing is beneficial for reducing \hl{the} overfitting by averaging multiple decision trees\hl{, as demonstrated} in this study. \hl{As a result, the use of RF algorithm enabled to reduce} data processing time and \hl{to increase the} accuracy of \hl{the }Landsat \hl{8-9} OLI/TIRS imagery}. 

%----------- subsection ----------->
%\pagebreak
\section{Study~Area}

This study \hl{is located in }the coastal \hl{area} of Mozambique\hl{, }western \hl{part} of the Indian Ocean{\hl{. Specifically, it investigates the region of the} Bight of Sofala, where the estuary of the Beira river enters the Mozambique Channel}, Figure~\ref{fig01}. 
...
\hl{As} one of the developing countries of Africa, Mozambique is notable for rapid land cover changes\hl{ }related to the environmental effects and urbanisation{. As a result, }landscape dynamics has \hl{recently }become one of the major issues of the country\hl{, }which is reported in many \hl{relevant} studies~\cite{BACAR2023,JANSEN2008308,MUCOVA2018e00447}. Located on the southeast coast of Africa, Mozambique presents a country with \hl{diversified topographic forms} and landscape setting\hl{. The most remarkable geomorphic features} include a complex mix of \hl{the} inland hills and low plateaus \hl{situated }along the narrow coastal strip \hl{which contrast with} rugged highlands \hl{located }in the western parts of the country. {Such} variability of landscapes \hl{favours diversified }vegetation {structure, increases \hl{the }biodiversity} richness and {creates }\hl{unique} land cover \hl{patterns} {which are }characteristic for{ Mozambique}, Figure~\ref{fig02}. 
...
Major geomorphic types \hl{of Mozambique }include numerous highlands interspersed with plateau, covered with woodlands, and~the lowlands {which are }situated mostly to the south of the Zambezi River{---}the major river of \hl{the country}. \hl{The }Zambezi \hl{River }roughly {separates} the country into the northern and southern parts {that have notably }distinct ecosystems. Being the fourth-longest river in Africa with complex hydrological setting, it largely affects the distribution of plants and species in the surrounding landscapes and \hl{in the }estuary zone\hl{, which is }located to the north of the Bight of Sofala~\cite{Dunham,Dunham1991,Simasiku}. The~hydrology of \hl{the} Zambezi River is connected to the main channel and tributaries and water management from the eight surrounding countries of the basin~\cite{Mbumwae}. {Thus}, through the downstream and upstream of waters that change local salinity and suspended sediments, the~hydrology of Zambezi strongly influences the coastal ecosystems of the Sofala Bank~\cite{NEHAMA2021101891}. {Nevertheless, recent construction of \hl{the} artificial} dams have had significant environmental effects on the major floodplains\hl{. For instance, these include} the \hl{reduced} water supply and river discharge~\cite{Gope} \hl{and increased} sedimentation of the coastal landscapes of its estuary~\cite{Moore}. 

\hl{The coastal ecosystems in the} Bight of Sofala \hl{are} one of the \hl{most} environmentally vulnerable regions \hl{in} Mozambique. This is caused by\hl{ }multiple impacts \hl{from} climate \hl{setting }and \hl{tidal hydrology} \cite{LEAL2009605} \hl{including tidal} cycles, shelf circulation and coastal currents\hl{ }~\cite{MALAUENE20181}. In~turn, the~hydrological setting of coastal ecosystems influence \hl{patterns of }primary production, distribution of zooplankton, phytoplankton and nutrients in the \hl{shelf} waters~\cite{MALAUENE2021107268}. Besides, rapid expansion of \hl{urban} areas \hl{caused by the} population \hl{growth }and economic pressure\hl{ changes landscapes}{. Such social processes }\hl{result} in \hl{urban sprawl which changes }natural landscapes \hl{into the artificial spaces }\cite{JOKARARSANJANI201838}. Moreover, the~Sofala Bank is known for the industrial shallow-water shrimp fishery~\cite{SOUSA2006207,MIGUEL2003365,GAMMELSROD199291} which creates additional pressure on the coastal ecosystems through\hl{ }overexploitation of natural resources. 

\hl{The}~repeated and severe floods and cyclones in\hl{ }western part of the Indian Ocean have a great influence on{ }landscapes and \hl{distribution of }vegetation \hl{along the coasts of} Mozambique. As~a result, Mozambique is reported to be one of the most vulnerable south African countries{\hl{ }to} flooding and tropical cyclones{. Such climatic hazards }cause\hl{ }damage to {the }infrastructure and {create additional factors to the disruption of }ecosystems~\cite{NHANGUMBE2023101015,SALVUCCI2020106713,Gall}. {Another\hl{ }}environmental {issues in} coastal \hl{regions} of Mozambique \hl{concerns }the decline of mangroves \hl{{which} is caused by the cumulative effects from }the anthropogenic activities and climate change~\cite{COME2023106813}. {At the same time, }\hl{as }highly productive coastal ecosystems with unique fauna and flora\hl{, }mangrove swamps are important source of natural resources and livelihoods for local \hl{people}. Therefore, their decline and degradation negatively affect the sustainability of coastal ecosystems\hl{.}

{Over\hl{ }recent decades, the~}consequences of floods in Mozambique include the destructions in landscape structures~\cite{FICHTNER2023103329}\hl{ }{and} crops during the flood\hl{ }{period}~\cite{BOFANA2022112808}. {Moreover, \hl{the }increased }landscape fragmentation {resulted in physical disintegration of \hl{the }continuous habitats into smaller patches with decreased}\hl{ }size and isolation {between\hl{ }habitat clusters. The~consequences of such processes negatively affect }biodiversity and species richness~\cite{Guldemond}\hl{ }as~well as lead to the disruptions in coastal ecosystems~\cite{RAMAYANTI20221025}. In~social-economic aspects of Mozambique, climate-related natural hazards and disrupted landscapes affected farming sector, fishery\hl{, }and~agriculture activities in the coastal \hl{areas} of~Mozambique. 

All these factors must \hl{are required for effective} environmental monitoring system\hl{. Specifically for }the coastal regions of Mozambique\hl{, data analysis enables }to define the cost of landscape restoration in the affected areas and {to }\hl{detect} land cover\hl{ }changes.{ }\hl{Proper land} planning in \hl{the }coastal areas of Mozambique requires {costly process of }evaluating \hl{landscape }dynamics\hl{, }\hl{climate change} {assessment} and \hl{integrated} socio-environmental \hl{analysis}~\cite{SMITH2019101976,LUNDGREN2022103023,SILVA201843,MARTINS2022102793}.{ The }lowest possible cost of \hl{monitoring} coastal areas of Mozambique \hl{can} be achieved by evaluating the difference between{ }\hl{land cover} types and patch analysis using image classification and quantification of landscape fragments~\cite{PITTMAN2023}. This is possible using cartographic mapping and time series analysis of the satellite images~\cite{XI2019111212,ABIDI2023106152,MOHAMMADI2023272}. Land surface topography which varies significantly in Mozambique influences the distribution of the vegetation types which can be considered in agricultural planning, environmental management \hl{and} ecological monitoring of the coastal~regions. For~instance, to~preserve\hl{ }vulnerable ecosystem of the narrow strip along the coasts of the Bight of Sofala, operative monitoring using remote sensing data \hl{is }a useful technical tool~\cite{NAMAGANDA2023117811}. 
...
\subsection{Workflow}
In this paper, {the} multi-temporal Landsat \hl{8-9} OLI/TIRS images {have been} used for land cover mapping in the coastal region of the Bight of Sofala, eastern Mozambique,\hl{ }Indian Ocean. {The\hl{ }methodology included the \hl{extraction of the} training data from\hl{ }raster images, supervised ML using ensemble classification\hl{, }regression tree method by RF and cross-validation tasks}. {The schematic view of the workflow in cartographic and remote sensing \hl{tasks} is portrayed in Figure} \ref{fig03}\hl{.}
...
The workflow is {completed using GRASS GIS software and} \hl{includes} the following {14} steps:

\begin{enumerate}
  \item Image import and interpretation {using Geospatial Data Abstraction Library (GDAL)\hl{ }and reprojecting the images\hl{. }Since all the images must be registered in the software with a coherency to each other, the~registration procedure of the Landsat images was performed using the} `r.import' module. The~metadata of the satellite images were checked for assessment of suitability and quality control: data acquisition, cloudiness\hl{, }sun elevation {and \hl{geographic} extent \hl{which }was set up to the current region. Technically, this was done using \hl{the }code ``r.import input\hl{=}%MDPI: Please check the full text and confirm if space is needed around equal signs. % Author's reply: No, not needed.
/Users/<path>/ImageName.TIF output=ImageName extent=region resolution=region''. Here the path to the original image is provided in full; the resolution is set to the 30 m as for Landsat scenes}.
  \item Image preprocessing is a fundamental approach in remote sensing data analysis\hl{. It  aims} to prepare the images for further processing and analysis. Here, the Landsat images were integrated with GDAL and preprocessed using the `i.landsat.toar'\hl{ }module which calculates the top-of-atmosphere spectral reflectance and temperature for Landsat OLI/TIRS scenes. To~this end, the~information on sun elevation and the date of image acquisition \hl{were} used from the metadata (product date). The~Digital Numbers (DN) of the image pixels were converted to the Top-of-Atmosphere Radiances (ToAR). {The resulting output images \hl{were }processed using the `i.landsat.toar' module \hl{and} transformed \hl{the }DN of \hl{the} images which\hl{ }calibrated \hl{the} top-of-atmosphere~reflectance.}
...
''. Here, each band was selected for each of the RGB triplets (red, green and blue) which enabled to visualise the images in various combinations (natural and false colour composites)}. 
  \item {The landscapes of coastal Mozambique are characterised by a complex mosaic of heterogeneous land cover types that\hl{ }exhibit structural similarities in spectral reflectance. Therefore, }radiometric {partition of images was performed} using unsupervised k-means method {by the }`i.cluster' module {which employs computer vision techniques for objective and automatic recognition of land cover types. This method generates spectral signatures for different land cover types using clustering algorithm that identifies the differences between various values of spectral reflectance for each pixels and assigns \hl{them} to classes accordingly.}
    \item Generating signature files for land cover types {was performed }using \hl{the }'signaturefile' function of the `i.cluster' module. \hl{The profiles of the }spectral backscatter coefficient\hl{ }of the land cover types were generated and compared to {the }FAO classification with 10 major \hl{identified} land cover classes{\hl{ }in \hl{the }coastal regions of Mozambique}. \hl{Various} characteristics of landscape patches were identified and compared for\hl{ }images taken at different {periods as a time series}\hl{.}
  \item Image classification {\hl{was done to identify }land cover classes }using the Maximum Likelihood method (`i.maxlike' module). {This method was selected due to its versatility and ability to produce fast and accurate results. In~essence, the~MaxLike classification algorithm uses clusters generated in previous step and classifies pixels\hl{ }in an iterative self-organising process of image partition to produce a set of spectral classes. \hl{Afterwards, the}  labels \hl{are assigned }to the clusters automatically and\hl{ }maps \hl{are generated }using a defined colour palette}.
  \item Generating training pixels from an older land cover classification using `r.random' module \hl{was done to generate }a set of raster point maps. {This module creates a raster map which contains coordinates of points whose locations have been randomly determined to ensure \hl{the }objectivity and randomness of the representative set of pixels.\hl{ }It } includes randomly located cells and pixel points using \hl{a} non-deterministic random seed. {Technically, it was implemented by the following code: ``r.random input=L8CL seed=100 npoints=1000 raster=L8CLroi''.}
  \item Creating the imagery group with all Landsat OLI/TIRS {multispectral }bands {that have 30 m resolution\hl{, }excluding panchromatic bands. This was performed }using `i.group' module {by the following code: ``i.group group=L82015 input=L801,<...>,L807''}. 
  ...
  \item Performing prediction using `r.learn.predict' module of GRASS GIS{. This is the essential part of the ML data processing }which uses a fitted Scikit-Learn estimator {derived} from Python to raster layers in a group of images. {The implementation is performed using the code: ``r.learn.predict group=L8\_2015 load\_model=rf\_model.gz output=rf\_classification --overwrite''. \hl{The} algorithm uses\hl{ }training \hl{dataset} generated in \hl{a }previous step and \hl{applies} the created model ``rf\_model.gz''. \hl{The}~main goal\hl{ }is to apply a fitted estimator from the Python's ML library Scikit-Learn to \hl{the imagery group }which was generated during the step ``i.group''.} 
  \item \hl{Checking} raster categories using \hl{the }module `r.category'\hl{. }{The category values and labels \hl{are displayed }for\hl{ }raster layer requested for the generated images\hl{. This was done }by the command ``r.category rf\_classification'' \hl{which} reports the data in the console output. In} this case, 10 major land cover classes of the coastal area of Mozambique were identified by the computer vision, which are automatically applied to the classification results.
  \item Visualization of the results {was performed }using {the GRASS GIS }module `r.colors' which copies \hl{the }colour scheme from \hl{the }land class training map to the output maps{. For~instance, the~code for the maps of Random Forest classification \hl{was done} as follows: ``r.colors rf\_classification color=plasma-e''}
  ...
  \item \hl{Interpretation} of the results {was based on the} comparison of maps of land cover types of the Bight of Sofala, Eastern Mozambique for 2015, 2018 and 2023 {to analyse spatio-temporal dynamics of land cover types in the coastal landscapes}. 
\end{enumerate}  

%----------- subsection ----------->
\subsection{Data}

In this study, three multi-temporal images of the Landsat Operational Land Imager and Thermal Infrared Sensor (OLI/TIRS) were selected {to produce classification \hl{using }machine learning\hl{. }The~images were chosen }due to the high quality of the data, availability {of the\hl{ }high-quality 'leaf-on' data on coastal Mozambique }and coverage of {the study area}. The~{most essential }technical characteristics of the images are summarised in Table~\ref{tab01}. 
...
The {values of the }cloud {coverage are} 0.01, 0.11 and 0.69 for the images on 2015, 2018 and 2023, respectively. The~original satellite images are stored in Universal Transverse Mercator (UTM) Zone 36 for Mozambique. The~terrain data of the study area used for topographic visualization is based on the General Bathymetric Chart of the Oceans (GEBCO) grid with high spatial resolution and accuracy (15 arc seconds) and visualized as the digital terrain model Shuttle Radar Topography Mission (SRTM). The~topographic names are added \hl{according} to the gazetteer of Digital Chart of the World (DCW).  

\subsection{Software}

The traditional image processing methods rely on the use of GIS~\cite{GEMUSSE2023101022,LIANG2023113367,BEY2020111611}. However, such methods have some drawbacks in that{ }the human-controlled maintenance of workflow is required for\hl{ }image\hl{ }processing, classification and analysis, which might be hard\hl{ }in \hl{case of } \hl{image series}. In~contrast, script-based image processing \hl{uses} automation of data processing \hl{to derive} information \hl{from the} RS data~\cite{Lemenkova202213}. An~alternative approach is presented by machine learning (ML)\hl{ }which can be used as descriptor\hl{ }of pixel values\hl{ }and partial replacement of the traditional methods using programming~\cite{jimaging9050098,Lemenkova202229}. The~ML\hl{ }is based on {the} algorithms of artificial intelligence which include\hl{ }computer vision and statistical \hl{analysis}. \hl{This} study applies the ML\hl{ using} the latest available version of GRASS GIS 8.3.1\hl{.}

\hl{The}~installation of the program was performed using {the} `sudo port install grass' with compilation {that includes }the\hl{ }dependancies {of} the essential Python's libraries and\hl{ }ML packages. \hl{The }programming is \hl{implemented} in \hl{the }GRASS GIS as scripts\hl{. This automates} the repeated steps of image processing {to improve} the workflow~\cite{technologies11020046,jmse11040871}. The~data processing was performed on the MacBook Apple Sonoma (arm64 {architecture}). The~Cooperative Computing Tools (CCTools) which enable large scale distributed computations from clusters were updated and installed using \hl{the }sudo {command} as follows: `sudo port clean cctools' and `sudo port \hl{-v} %MDPI:  Please check the full text and confirm if space should be removed before hyphen. % Author's reply: No, space should be kept: it is a syntax of programming. 
...
The advanced script-based cartographic ML methods of \hl{the }GRASS GIS for monitoring and mapping land cover types are built on the growing demand for the time- and cost-effective approaches in cartographic workflow and remote sensing data analysis\hl{. It enables} to save time and resources while processing \hl{the }geospatial datasets, and~{to }use open source satellite images. \hl{Three} major script-based toolsets were used in this study for geospatial data analysis and environmental monitoring of West Africa:
 ...
 \begin{enumerate}
        ...
        \item GRASS GIS \hl{modules} of `i.cluster', `i.maxlik'  were used for unsupervised image classification and accuracy assessment \hl{by} computed rejection probability classes for classified pixels, Listing \ref{lst03};  
        ...
        \item GRASS GIS \hl{module} `r.learn.train' was used for Random Forest classification of Machine Learning (ML)-based supervised image classification using training data generated in previous step, Listing \ref{lst05}. 
  \end{enumerate}
  
The main idea \hl{of this framework }is to \hl{perform} in a single workflow\hl{ }both\hl{ }image processing and\hl{ }mapping using scripts run from the console. This enabled to reveal the technical performance of \hl{the }automated geospatial data processing using programming approach of three various programming suits \hl{of} the script-based software. Such approach demonstrated\hl{ }that the inclusion of scripts speeds up cartographic plotting and image processing and \hl{facilitates} research methodology. As~mentioned earlier~\cite{Lemenkova202215}, the~main advantage of scripts consists in automation of the process and repeatability of the workflow\hl{ which saves} time. \hl{Using} the information received from the automated image analysis and interpretation\hl{, }we were able to detect the changes in the analysis landscapes of eastern Mozambique for evaluation of environmental effects from climate change\hl{. This was achieved }through \hl{time }series \hl{analysis }of the \hl{satellite }images\hl{ }taken on different years (2015--2023). 
...
{The }accuracy assessment for the unsupervised classification was performed using the 'reject' function of the `i.maxlik' modules of GRASS GIS which calculates the output raster map holding {the} reject threshold results. The~aim of this step is to evaluate the confidence level at which each cell in the raster image is categorised during the classification process. {\hl{The} algorithm} generates the reject threshold map layer which contains the index to a computed confidence level for each classified pixel in the\hl{ }satellite scene. The~{predefined confidence} intervals \hl{include} sixteen \hl{values}. The~rejection probability image indicated the values of pixels between the acceptable at one, i.e.,~the pixel is kept up to 16 which means that the pixel is~rejected.

The final part of the workflow included the Machine Learning (ML) approach of Random Forest (RF) classification of GRASS GIS which was performed using a set of modules\hl{: }`r.random', `r.learn.train', `r.learn.predict', `r.category' and auxiliary modules of data processing and visualization,\hl{ }Listing \ref{lst05}. 
...
The~training of a Random Forest classification model was performed using `r.learn.train'. The~training {data} were generated from {the \hl{previous }step of}\hl{ }land cover classification{. \hl{The} scheme of} \hl{the }training pixels was applied to perform {the} classification on the target {satellite }image. The~prediction was performed using the `r.learn.predict' module {of the GRASS GIS}. 

%----------- section ----------->
\section{Results}

Colour composites created using a combination of bands for false and natural colour composites are shown in Figure~\ref{fig05} with a case of image on 2015. The~image dates are distributed throughout the important stages of the vegetation growth period, in~order to obtain temporal profiles of each kind of \hl{the} land cover types \hl{to analyse} the separability among classes. {The particular characteristics of Mozambique consists in its longitudinal extent with \hl{the} coastline stretching for ca. 2000 km\hl{, }11\degree~S to 27\degree~S. Besides, the~effects from tropical ocean currents \hl{directed} southwards along the length of the country \hl{influence} environmental setting. Therefore, regional characteristics naturally differ along the country. 
...
\hl{Weather} patterns \hl{in} Mozambique \hl{vary} due to the global warming and increase in temperature. Nevertheless,\hl{ }the~whole country broadly follows a southern African weather pattern\hl{:}~\hl{ }wet period with heavy rainy between November and March and dry period\hl{ }from April to late November. }Since Mozambique has a tropical climate which has two distinct seasons, the~images were taken during the dry period. \hl{Better} cloudiness conditions \hl{and} dense vegetation~coverage are observed\hl{ during }dry season\hl{ as shown on} the~images\hl{.}  

The results of the classification {are} shown in Figure~\ref{fig06}{. They }were used as training data for the next step of Random Forest (RF)\hl{ modelling }{which requires} seed data. The~land cover types\hl{ }include {the following classes: }(1) mosaic cropland vegetation, (2) artificial surfaces and associated areas; (3) rainfed croplands; (4) open grasslands; (5) sparse vegetation; (6) broadleaved deciduous woodland; (7) mosaic forests, grassland and shrubland; (8) water areas; (9) needle leaved deciduous evergreen forests; (10) repeatedly or permanently flooded lands in brackish waters with broadleaved forests or shrub lands. {The }coastal areas are characterised by\hl{ }grassland or woody vegetation distributed on {areas of }\hl{ }soil regularly flooded \hl{by} fresh, brackish or saline~water. 

Figure~\ref{fig07} shows the result of a chi square test on accuracy \hl{assessment} of the classification on each discriminant result at various threshold levels of confidence. {The aim of the chi square test is to determine if the correlation exists between two qualitative variables \hl{and} if the existing associations are statistically significant. }When using the Maximum Likelihood classification method to classify the land cover types, the~{accuracy of the classification} depends on the quality of the training dataset and technical parameters of the {satellite }image. Thus, evaluating the accuracy analysis,\hl{ }{presents an }important {step in \hl{classification}}. The~approach to accuracy assessment\hl{ }is based on the function embedded in `i.maxlik' where\hl{ }pixels are classified according to the probability of their correct assignment to \hl{each} land cover~class\hl{.} 
...
This method is suitable for providing \hl{the} sufficient estimation on the accuracy of training samples that were used for the final steps of {classification using Random Forest}\hl{. }The resulting images of Random Forest classification of Landsat \hl{8-9} OLI/TIRS images by ML approach of GRASS GIS for years 2015, 2018 and 2023 are presented in Figure~\ref{fig08}. 
...
Figure~\ref{fig08} shows the {results of the land cover types }classification of the coastal region of Mozambique {performed }using RF classifier approach of ML method {in} GRASS GIS. The~land cover types in Mozambique were selected for Random Forest classification with the estimator settings tab providing {the }access to the most pertinent parameters{. These} affect \hl{the} {previously} described algorithms where\hl{ }landscape patches were classified using {the\hl{ }}Maximal Likelihood {classifier}\hl{.} \hl{The}~embedded Scikit-Learn library of Python contents \hl{the} estimator parameters that are supplied for classification. \hl{These} parameters were tuned using a grid-search in the Landsat raster maps by inputting multiple regressors\hl{. This enabled }to indicate the representativeness of {the }land cover classes, uniformity or heterogeneity of patches and the classification of pixels located far from the edges of the raster~scene. 

{The}\hl{ }ML approach{ }by Scikit-Learn library of Python \hl{ensures} that the {confusion of} image {pixels} is avoided as much as possible compared to the traditional classification methods. Moreover,\hl{ }training pixels generated during clustering and used to perform \hl{the} RF-based classification separated the images into \hl{clusters} of pixels. These groups were used both as \hl{a} decision tree training seed dataset for Random Forest classifiers, and~as \hl{a} post-classification accuracy assessment. In~this way, the~RF method avoided biased reference information of the group of~pixels.

%----------- section ----------->
\section{Discussion}

In this study, the~ML method of Random Forest classification of GRASS GIS was applied for {processing and analysis of }three Landsat 8-9 OLI/TIRS images {with the aim of }land cover mapping {in} the coastal region of the Bight of Sofala, Mozambique. Quantitative image processing and interpretation is not an easy {task} due to the complex function of {multiple factors. These include, for~instance, }technical parameters of the image (sun, sensor, target geometry), regional environmental effects (cloudiness, wetness, {heterogeneity, }diversification and fragmentation of landscapes,\hl{ }topographic ruggedness and slope curvature) \hl{as well as} sensitivity to leaf characteristics~\cite{Mitchard}. The~ML approach of GRASS GIS enabled\hl{ }to perform quantitative image classification using Random Forest algorithm\hl{ }which evaluated the similarity of pixels and \hl{assigned} them into classes using \hl{the} `r.learn.train' module. \hl{The} statistical information \hl{was obtained }from the raster data analysis \hl{to }represent\hl{ }the land cover types in Mozambique for{ }years 2015, 2018 and~2023. 

\hl{Such} algorithm enabled to model\hl{ }clusters of pixels\hl{, }which {were} used to compare the climate effects {that cause} changes in land cover classes and {lead to spatio-temporal }landscape {dynamics}. This was achieved using the ensemble classification {achieved by the approach of Random Forest}\hl{. }Each tree in {this} ensemble is based on a random subsample of the training data that were initially obtained using {the }unsupervised classification as a part of data processing. This work is comparable with investigations done by {the }related works{, for~instance,} in~\cite{FERNANDO2023,KOLARIK2024111445}. 

\hl{Similar research} ~\cite{BERA20234691,land12122149,ZHANG2023108250} investigated the complementarities of the ML approach for mapping coastal regions using a set of {the }multi-resolution imagery Sentinel-2 Multispectral Instrument (MSI) and Landsat-8 OLI. To~continue these studies, the~experiments on image processing using Random Forest approach reported in this article have investigated the landscape dynamics in study area of eastern Mozambique\hl{. This was achieved }using randomly-selected subset of the predictors available during each node split in the ML-based classification method of GRASS GIS. Other studies~\cite{MOUNTRAKIS2023106,SINGH2021100645,ZHENG2023100974} used a neural network {as a technical approach to classification of} vegetation and land cover {types. Such }technique {of} \hl{the} remote sensing data processing {\hl{aims} at classification of the }land cover types using ML methods and \hl{reports} high accuracy. {Hence, \hl{we used the} emerging technologies of scripts and programming algorithms for image processing through a \hl{set} of \hl{GRASS GIS} modules\hl{.}}

\hl{The demonstrated} workflow \hl{was} developed using a short series of \hl{the }Landsat images. These images had various combinations of pixels which were evaluated by the ML\hl{ }methods to analyse\hl{ }land cover \hl{changes }using prediction analysis. \hl{Each} tree of the algorithm produced a prediction\hl{. The} final result of image analysis \hl{was}obtained by selecting the mean values across all of the trees in the Landsat image raster using computer vision. In~other studies, a~decision tree classifier was adopted to \hl{the} RS data processing for automatic classification using diverse software. For~instance, ref.~\cite{BROWNDECOLSTOUN2003316} used the ensemble techniques of boosting and consensus filtering of the training data to improve the quality of the training dataset. Ref.~\cite{VAIDYA2024101770} compared the integrated use of the three classification schemes based on RF, DL neural network and convolutional neural network to distinguish urban, natural and heterogeneous landscape patches. Likewise\hl{,} the~ExtraTreesClassifier has been used as a\hl{ }variant of the GRASS GIS ML algorithm. Here each node split during\hl{ }image processing includes the selected splitting rule which is based on the most optimal~solution. 

Such optimization {applied to} several randomly-generated thresholds by the Random Forest method {presented} advantages over the traditional unsupervised methods of image classification when generating classes for image classification. For~instance, ref.~\cite{PASQUARELLA2023103561} reports that \hl{the} parameter adjusted decision tree ensembles and {RF} ensembles performed well for image segmentation and supervised and outperformed other approaches. Likewise, the~\hl{updates} in the algorithms were reported by using {the }improved decision tree algorithm based on{ }fuzzy approach\hl{ }for technical improvement of the\hl{ }performance~\cite{ALOBEIDAT20151192}. Although~the study presented here is not intended to suggest that scripting methods replace the traditional GIS, the~integration of scripting workflow of GRASS GIS with satellite image analysis\hl{ }and cartographic plotting provides a more effective approach. \hl{This} study demonstrated that the programming codes\hl{ }supplement the traditional cartographic methods used in the remote sensing software for monitoring coastal areas. Moreover, this work illustrated how the analysis of land cover changes can be visualised in the automatic way using ML algorithms of GRASS GIS. 
%----------- section ----------->
\section{Conclusions}

{\hl{Application of }programming for \hl{RS} data processing \hl{provides} new perspectives \hl{in} cartographic data processing. \hl{Such} approaches are useful for mapping complex landscapes in the entanglement of human-based factors and environmental processes that affect coastal ecosystems. In~view of this, current study presented advanced mapping approaches to spatial analysis and \hl{visualization} of \hl{the RS} data}. {The} use of the ML and intelligence classifiers of GRASS GIS increased the accuracy and precision of the produced thematic maps through the high level of automation. \hl{Moreover, it} improved the performance of tasks using diversified modules and application of RF algorithms for \hl{land cover} classifications. \hl{The}~{satellite }images were classified using {the }unsupervised and supervised\hl{ }methods of ML modules of GRASS GIS including Random Forest~classifier. 

The Bight of Sofala, located in the eastern coastal part of Mozambique is {selected} as a test area {due to the variability \hl{and} high heterogeneity of landscapes with a complex mosaic of landscape patches}. Ten types of land cover classes were identified on the {Landsat satellite }images {collected in years 2015, 2018 and 2023. The~data were }classified using \hl{the} Random Forest algorithm of machine learning (ML) methods supported by Scikit-Learn library of Python integrated with GRASS GIS. The~clustering approach of the machine-based classification was \hl{employed} for training and validation of the result of \hl{the} RF classification. The~{benefits} of the{ }\hl{RS} data{ used }for environmental monitoring {consists in the fact} that {they} provide the efficient way to map and visualise the remotely located regions\hl{ }such as coastal regions in East Africa. Scripts\hl{ }used as a\hl{ }methodological benchmark of \hl{the} automated image processing and cartographic workflow enabled to focus on the analysis of the coastal landscapes of Mozambique\hl{.} 

The coastal region of the Bight of Sofala and {its }surroundings were analysed {using} the classified series of {satellite }images {Landsat by applied methods of }automated data analysis of ML approach. The~{advantage of the }proposed algorithms of GRASS GIS scripts {consists in their repeatability and applicability. }\hl{The use of the }supervised classification by `r.learn.train' module can be\hl{ }extended to other regions and environmental applications where satellite image processing is required{. In~this way, the \hl{ML} method} classification of landscapes{\hl{ }can be applied in\hl{ }studies that use} multi-temporal set of\hl{ }images. The~advanced approach of GRASS GIS scripts was introduced to evaluate the environmental properties of the coastal landscapes of Mozambique\hl{. }Changes in land cover types caused by climate effects in East Africa were visualised using image processing by GRASS GIS modules of `r.learn.train' , `i.cluster', `i.maxlik' and other auxiliary~modules. 

The\hl{ }results {visualise} the {dynamical }behaviour of land cover types in the coastal landscapes of Mozambique\hl{ caused by} the climate effects \hl{which act} as \hl{drivers} for desertification and land degradation in\hl{ }costal ecosystems {of East Africa}. \hl{We} also discussed the ways in which the ML methods {technically }contrast with {the }traditional methods of {the }unsupervised classification{. For~instance, these use} k-means clustering {as an approach} to image analysis{ }for evaluating the landscape changes using a series of images taken on different dates\hl{. }Thus, {it has been demonstrated that }the ML approach of GRASS GIS{ has a }high automation in sequential image processing workflow which enables {the }effective use of RS data for environmental monitoring of coastal~regions.
