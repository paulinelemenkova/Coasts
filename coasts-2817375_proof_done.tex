...
\caption{Study %MDPI: Please change the hyphen (-) into a minus sign ($-$, "U+2212"). e.g., "-1" should be "$-$1". % Author's reply: the map in figure 1 is corrected as requested (the hyphen (-) is changed into a minus sign ($-$, "U+2212")).
 area in the coastal region of the Bight of Sofala, Mozambique is indicated by the cyan-colored rotated square. Mapping software: Generic Mapping Tools, version 6.4.0, creator: \cite{Wessel}, location: Palisades, NY 10964, United States. %MDPI: Please state the software version number, creator, and the location (city, country) from where the software was sourced. %Author's reply: information is added.
 (GMT). Map source:~author.}\label{fig01}
\end{figure}
...
\caption{Land %MDPI: Please change the hyphen (-) into a minus sign ($-$, "U+2212"). e.g., "-1" should be "$-$1". % Author's reply: corrected: the hyphen (-) is changed into a minus sign ($-$, "U+2212")
 cover types in Mozambique according to the Food and Agriculture Organization (FAO) data. Mapping software: QGIS, version 3.34, creator: QGIS Development Team, location: London, England, U.K. %MDPI: Please state the software version number, creator, and the location (city, country) from where the software was sourced. % Author's reply: ok, information is included.
Map source: author's~work.}\label{fig02}
\end{figure}
...
\end{adjustwidth}
\caption{{Workflow of the data processing. Diagram software: R, software version number 4.2.2, creator: R Core Team, location: Vienna, Austria. %MDPI: Please state the software version number, creator, and the location (city, country) from where the software was sourced.  % Author's reply: the information is added.
 Source: author's work.}}\label{fig03}
\end{figure}
...
''. Here, each band was selected for each of the RGB triplets (red, green, and blue), which enabled us to visualize the images in various combinations (natural and false color composites)}. 
  ...
  \item The \textls[-10]{creation of an imagery group with all Landsat OLI/TIRS {multispectral }bands {that have 30 m resolution{, }excluding panchromatic bands: this was performed }using `i.group' module {with the following code: ``i.group group=L82015 input=L801,<...>,L807''}. }
...
\begin{listing}[H]
\caption{GRASS %MDPI: The captions of listings 3 and 4 are the same (GRASS GIS code for grouping bands and clustering.), Please confirm if it is correct and need to be modified. % Author's reply: caption for Listing 4 is corrected to "GRASS GIS code for data display, visualization and mapping". Many thanks for noticing it !
 GIS code for grouping bands and clustering.} 
\rule{\columnwidth}{1pt}
...
\begin{listing}[H]
\caption{GRASS GIS code for data display, visualization and mapping.} 
\rule{\columnwidth}{1pt}
...
\newblock {\em Ecol. Inform.} {\bf 2022}, {\em 69},~101678.
...
\newblock {\em Ecol. Inform.} {\bf 2013}, {\em 17},~82--93. %MDPI: We removed the redundant issue content, please confirm. % Author's reply: yes, confirm.
...
\newblock {\em Geophys. Res. Lett.} {\bf 2009}, {\em 36%MDPI: Please add the page number.% Author: added (1-6)
},~1--6.
